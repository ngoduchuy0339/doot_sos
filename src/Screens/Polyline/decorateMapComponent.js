...
export const createNotSupportedComponent = message => () => {
  console.error(message); // Vô hiệu hoá dòng không có điều kiện
  return null;
};
...
export default function decorateMapComponent(Component, { componentType, providers }) {
  ...
  Component.contextTypes = contextTypes; // tránh vô hiệu hóa dòng không chỉ định lại thông số

  // tránh vô hiệu hóa dòng tiếp theo mà không chỉ định lại thông số
  Component.prototype.getAirComponent = function getAirComponent() {
    ...
    if (platformSupport === NOT_SUPPORTED) {
      components[provider] = createNotSupportedComponent(`react-native-maps: ${componentName} is not supported on ${Platform.OS}`); // tránh vô hiệu hoá max-length của dòng
  ...
  Component.prototype.getUIManagerCommand = function getUIManagerCommand(name) {  // tránh vô hiệu hóa dòng không chỉ định lại thông số, độ dài tối đa max-len
  ...
  Component.prototype.getMapManagerCommand = function getMapManagerCommand(name) { // tránh vô hiệu hóa dòng không chỉ định lại thông số, độ dài tối đa max-len
